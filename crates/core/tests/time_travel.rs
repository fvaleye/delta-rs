...
async fn time_travel_by_ds() {
    ...
    let table_url = Url::from_directory_path(Path::new("../test/tests/data/simple_table")).unwrap();
...
async fn time_travel_by_ds_checkpoint_vacuumed() {
    ...
    let table_url =
        Url::from_directory_path(Path::new("../test/tests/data/checkpoints_vacuumed")).unwrap();
