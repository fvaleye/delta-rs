        ...
        Box::pin(
            async move {
                ...
                let source_schema: Arc<Schema> =
                    normalize_for_delta(&Arc::new(source.schema().as_arrow().clone()));

                // If source schema was normalized (e.g. Date64→Date32), add a cast projection
                if source_schema.as_ref() != source.schema().as_arrow() {
...
mod tests {
    ...
    async fn test_write_date64_normalizes_to_date32() {
        ...
        // Table schema should have Date32 (Delta's date type), not Date64
    ...
    async fn test_append_date64_to_existing_date32_table() {
        ...
        // Verify can read back both rows
        let batches = get_data(&table).await;
        let total_rows: usize = batches.iter().map(|b| b.num_rows()).sum();
